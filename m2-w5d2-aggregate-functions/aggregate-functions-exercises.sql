...
SELECT SUM(surfacearea * .01) as sum_surface_area, continent
FROM country
GROUP BY continent
...
SELECT (gnp - gnpold) AS gnp_difference, ABS(gnp - gnpold) absolute_value_gnp_difference, name
FROM country
...
SELECT AVG(lifeexpectancy) as average_life_expectancy, MIN(lifeexpectancy) as min_life_expectancy, MAX(lifeexpectancy) as max_life_expectancy, continent
FROM country
GROUP BY continent
