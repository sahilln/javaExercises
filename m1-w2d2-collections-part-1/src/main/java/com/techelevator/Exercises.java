...
public class Exercises {
	...
	public List<Double> arrayInt2ListDouble(int[] intArray) {
		List<Double> divideByTwo = new  ArrayList<Double>();
		for(double value: intArray){
			double divide = (value/2);
			divideByTwo.add(divide);
		}
		return divideByTwo;
	}
    ...
    ** INTERVIEW QUESTION **	 
	...
	public List<String> distinctValues(List<String> stringList) {
		...
		List<String> singleColor = new  ArrayList<String>();
		for (String color: stringList){
			values.add(color);
		}
		for(String single: values){
			singleColor.add(single);
		}
		return singleColor;
	}
	...
	public List<Integer> interleaveLists(List<Integer> listOne, List<Integer> listTwo) {
		List<Integer> interweave = new  ArrayList<Integer>();
		int i = 0;
		if(listOne.size() == listTwo.size()){
			for(int value: listOne){
				interweave.add(value);
				interweave.add(listTwo.get(i));
				i++;
			}
		}
		else{
			if(listOne.size() > listTwo.size()){
				for(int value: listTwo){
					interweave.add(listOne.get(i));
					interweave.add(value);
					i++;
				}
				interweave.add(listOne.get(listOne.size() - 1));
			}
			else{
				for(int value: listOne){
					interweave.add(value);
				...
				interweave.add(listTwo.get(listTwo.size() - 1));
			}
		}
		return interweave;
	}
	...
	public List<Integer> boardingGate(List<Integer> seatNumberList) {
		List<Integer> groupedSeating = new  ArrayList<Integer>();
		for (int value: seatNumberList){
			if(value >= 1 && value <= 10){
				groupedSeating.add(value);
			}
		}
		for (int value: seatNumberList){
